...
function parseTomlFrontmatter(content: string): { frontmatter: ZolaFrontmatter; body: string } | null {
  ...
  const frontmatter: ZolaFrontmatter = {
    title: '',
    date: '',
  };
  ...
  // Parse taxonomies
  const taxonomiesMatch = tomlContent.match(/\[taxonomies\]([\s\S]*?)(?=\[|$)/);
  if (taxonomiesMatch) {
    const taxonomiesContent = taxonomiesMatch[1];

    // Parse categories
    const categoriesMatch = taxonomiesContent.match(/categories\s*=\s*\[([\s\S]*?)\]/);
    if (categoriesMatch) {
      const categoriesStr = categoriesMatch[1];
      frontmatter.taxonomies = frontmatter.taxonomies || {};
    ...
    // Parse tags
    const tagsMatch = taxonomiesContent.match(/tags\s*=\s*\[([\s\S]*?)\]/);
    if (tagsMatch) {
...
function toYamlFrontmatter(fm: AstroFrontmatter): string {
  const lines: string[] = ['---'];
...
function convertFile(sourcePath: string, targetPath: string): boolean {
  try {
    ...
    const astroFrontmatter: AstroFrontmatter = {
