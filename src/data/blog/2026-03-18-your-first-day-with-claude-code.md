...
A colleague showed me his first Claude Code session last year. He'd opened a terminal in his home directory, typed a prompt, and was watching the agent happily read his `.env` files and get confused by three unrelated projects in adjacent folders. Two hours later he told me the technology was overhyped.

His setup was wrong. The agent wasn't bad; the environment around it was a trap.

I've seen that exact pattern dozens of times since. This post is the checklist I wish I'd had on day one.
...
The choice depends on your usage patterns, but one thing I'd stress: don't start with the free tier and judge the technology by the experience of a rate-limited model. Budget an amount and use the API if you are not sure about subscribing. The difference is not subtle.
...
Every agent session should run inside a container or isolated VM. No mounts to your host filesystem. No production tokens in environment variables. Dev-scoped everything. Sounds paranoid. It isn't.

A coding agent with access to your `.ssh` keys, `.aws` credentials, `.kube` config, and shell configuration is a security incident waiting to happen. Trail of Bits documents this explicitly. An agent reading a log file with a crafted prompt injection can generate a script that exfiltrates your credentials to an external server. This is not a hypothetical failure mode; it's Tuesday.
...
What most people miss is that without `/sandbox`, deny rules only block Claude's Read tool. A Bash command like `cat ~/.ssh/id_rsa` still works. The deny rules and the sandbox complement each other. Use both.
...
Five minutes on isolation saves the incident response you'll never have to write.
...
With the sandbox enabled, you can run in auto-allow mode: sandboxed Bash commands execute without permission prompts, while anything that tries to reach outside the boundary triggers a notification. This addresses what I'd call "approval fatigue", the pattern where constant "approve this command" prompts train you to click through reflexively. You get the worst of both worlds: interrupted flow and reduced security.

The sandbox doesn't replace container isolation for high-risk scenarios. The VM boundary is the security boundary. Everything else is defence-in-depth. For day-to-day development on trusted repositories, `/sandbox` with auto-allow gets you 80% of the security benefit with 5% of the setup cost. That's a trade-off I'm happy to make.
...
**Session management** is essential. You'll be running multiple Claude sessions simultaneously at some point. Use tmux or zellij for persistent sessions. When you close your laptop or lose connection, your sessions keep running. This is not a nice-to-have; it's infrastructure the moment you run parallel agents.
...
This is the single highest-impact configuration change you can make, and it takes two minutes. If you only do one thing from this entire post, set up your sandbox. If you do two, make this the second one.

Without LSP, Claude Code navigates your codebase using text search: Grep, Glob, and Read. It treats code as text. When you ask "where is this function defined?", Claude greps across the entire codebase. For a common symbol like `User`, that might mean hundreds of matches across dozens of files. Claude reads through each to narrow it down. Tens of seconds and many tokens consumed.

With LSP, the same query returns the exact file and line in milliseconds. This is the same code intelligence your IDE uses. Why wouldn't you give it to your agent?

The context window savings are significant. Instead of reading dozens of files to find a definition, Claude gets a precise answer and reads only the one file it needs.

The killer feature is self-correcting edits. After every file edit, the language server pushes diagnostics: type errors, missing imports, undefined variables. Claude sees them immediately and fixes them in the same turn, before you see the result. Without LSP, you'd get the result, try to compile, see errors, paste them back. Each round trip consumes more context. With LSP, that entire loop collapses into one step.
...
**Is there a testing framework in place?** If the agent can't verify its work, the quality drops dramatically. Give Claude a way to check its own work and quality improves noticeably. I'd go further: without tests, you're asking the agent to code blindfolded. Not everybody tests as much as they should; that's something to correct if you expect any success from your AI experiments.

## What NOT to Install

The pace of activity around Claude Code is extraordinary. Check the commit activity on awesome-claude-code and you'll see what I mean. There's a natural temptation to install everything that looks promising.
...
## Start Lean on Purpose

I have a setup I use and would recommend, but I am not listing it here on purpose. If you are starting with Claude, start minimal. The model is very capable on its own, and if you add a lot of tooling you may not get to understand what it can or can't do.

Add tools as you discover you need them. An MCP to let the model drive Chrome against your app. A skill or two to enforce TDD. Grow it incrementally.

The best setup is the one where you understand every piece and why it's there. My colleague, the one from the opening, eventually went back to Claude Code. He started with a container, an LSP, and nothing else. He hasn't called it overhyped since.
